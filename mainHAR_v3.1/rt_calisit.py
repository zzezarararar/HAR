...
if __name__ == "__main__":
    try:
        ...
        window_name = "Calibration stand sit"
    ...
    min_tracking_confidence=0.5) as pose:
            ...
            #esc
                ...
                if time_pass <= 5:
                    if type(avg_data) == list:
                        left_standing.append(abs(avg_data[23][1] - avg_data[25][1]))
                        right_standing.append(abs(avg_data[24][1] - avg_data[26][1]))
                        frame_count += 1
                ...
                if 5 < time_pass <= 9:
                    pass
                    cv2.putText(im_color,'Please sit down within 4 sec', (200,400) , cv2.FONT_HERSHEY_DUPLEX, 0.6, (0,255,255),2 , cv2.LINE_4)
                    cv2.putText(im_color,'Countdown ' +str(time_pass-5)+' sec', (200,500) , cv2.FONT_HERSHEY_DUPLEX, 0.6, (0,255,255),2 , cv2.LINE_4)
                if 9 < time_pass <= 13:
                    if type(avg_data) == list:
                        left_sitting.append(abs(avg_data[23][1] - avg_data[25][1]))
                        right_sitting.append(abs(avg_data[24][1] - avg_data[26][1]))
                        frame_count += 1
                        sitting_frame.append(frame_count)
                    cv2.putText(im_color,'Please sit still for 5 sec', (200,400) , cv2.FONT_HERSHEY_DUPLEX, 0.6, (0,255,255),2 , cv2.LINE_4)
                ...
                if cv2.waitKey(1) == 27 or time_pass > 14:
                    plt.plot(sitting_frame,left_sitting,'r--',sitting_frame,right_sitting,'bs',standing_frame,left_standing,'g--',standing_frame,right_standing,'ys')
                    break
        ...
        #Sitting data
        left_sit_peak,_ = find_peaks(left_sitting, height=0)
        right_sit_peak,_ = find_peaks(right_sitting, height=0)
        left_sit_data = [left_sitting[i] for i in left_sit_peak]
        right_sit_data = [right_sitting[i] for i in right_sit_peak]
        l_sit = max(left_sit_data)
        r_sit = max(right_sit_data)

        #Standind data
        left_stand_peak,_ = find_peaks([-i for i in left_standing], height=-10)
        right_stand_peak,_ = find_peaks([-i for i in right_standing], height=-10)
        left_stand_data = [left_standing[i] for i in left_stand_peak]
        right_stand_data = [right_standing[i] for i in right_stand_peak]
        l_stand = min(left_stand_data)
        r_stand = min(right_stand_data)

        #Print the calibration results
        print( l_sit+(abs(l_sit-l_stand))/4) #left
        print( r_sit+(abs(r_sit-r_stand))/4) #right
        plt.show()

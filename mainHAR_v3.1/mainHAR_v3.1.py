...
end = 0 # end the program flag
no_skele_frame_count = 0 # no skeletons frame counter
...
qt1 = []
qt2 = []
##durtion dur = [sit_dur,stand_dur,walk_dur,sleep_dur,Falling_dur,No activity_dur]
dur = [0,0,0,0,0,0,0]
act_index = [0,0,0,0,0,0]
##List for checking activity
activity_list = ['Sitting','Standing','Walking','Sleeping','Falling down','',' ']
## explode for plot
explodee=[0, 0, 0, 0]
...
## bed_boundary -> 4 corners of a bed => [x1,y1,z1],[x2,y2,z2],[x3,y3,z3],[x4,y4,z4]
...
walk_first_threshold = 0.024
walk_second_threshold = 0.035
...
def walking_standing(Qws):
    ...
    ## joint [[0] - neck, [11] = left shoulder, [12] = right shoudlder, [31] = right foot, [32] = left foot]
    joint_list = [0,11,12,31,32]
    ## deltaR: [0]-neck,[1]-left shoulder,[2]-right shoulder,[3]-left foot,[4]-right foot
    deltaR = [0,0,0]
    first_frame = [Qws[0][j] for j in joint_list]
    second_frame = [Qws[1][j] for j in joint_list]
    ## list structure -> [x1,y1,z1,x2,y2,z2]
    ## x1,y1,z1 correspond to joint coordinate in the first frame
    ## x2,y2,z2 correspond to joint coordinate in the second frame
    ...
    # check if the nose and foot joint coordinates are detected
    ...
    # check if the nose and shoulder joint coordinates are detected
...
def sleeping(joint_data_fil) :
    ...
    ## joint [[30] - right heel, [24] = right hip, [12] = right shoudlder]
    j_right = [30,24,12]
    ## joint [[29] - left heel, [23] = left hip, [11] = left shoudlder]
    j_left = [29,23,11]
    # Get Y-coordinate of joints
    joint_right = [joint_data_fil[j][1] for j in j_right]
    joint_left = [joint_data_fil[j][1] for j in j_left]
    # This condition is to get out from this activity
    if act == 'Sleeping' or act == 'Falling down':
        ...
        return(delta_r_shoulder_hip < sleep_s_h_threshold or delta_l_shoulder_hip < sleep_s_h_threshold)
    # This condition is to trigger this activity
    else :
        # If heels can be detected, use heel joints to estimate the activity
        if [type(a) for a in joint_right] == [np.float64 for i in range(3)] and [type(a) for a in joint_left] == [np.float64 for i in range(3)]:
            ...
            # Check if the delta R between joints on right side <= thresholds
            right_side = (delta_r_shoulder_hip <= sleep_s_h_threshold and delta_r_heel_hip <= sleep_f_h_threshold)
            # Check if the delta R between joints on left side <= thresholds
            left_side = (delta_l_shoulder_hip <= sleep_s_h_threshold and delta_l_heel_hip <= sleep_f_h_threshold)
            return(right_side or left_side)
        # If heels cannot be detected, use only shoulder and hip joints to estimate the activity
        else :
...
def sitting(joint_data_fil):
    ...
    deltaR_r = float(1)
    deltaR_l = float(1)
    # Get right side joints' coordinate and find deltaR
    joint_right = [joint_data_fil[j][1] for j in j_right]
    if [type(a) for a in joint_right]== [np.float64 for i in range(2)]:
        deltaR_r = abs(joint_right[1]-joint_right[0])
    # Get left side joints' coordinate and find deltaR
    joint_left = [joint_data_fil[j][1] for j in j_left]
    if [type(a) for a in joint_left]== [np.float64 for i in range(2)]:
        deltaR_l = abs(joint_left[1]-joint_left[0]) 
    
    return(deltaR_r < sit_right_threshold or deltaR_l < sit_left_threshold)
    
def check_bed_boundary(joint_data_filtered):
    global bed_boundary, in_bed
    x_pos = [] # list for X coordinate of each edge of a bed
    z_pos = [] # list for Z coordinate of each edge of a bed
    ...
    z_max = max(z_pos) 
    z_min = min(z_pos)
    xnose = [] # list for X coordinate of nose joint
    znose = [] # list for Z coordinate of nose joint
    xsh = [] # list for X coordinate of right shoulder joint
    zsh = [] # list for Z coordinate of right shoulder joint
    if  type(joint_data_filtered[0])== list :
        ## x nose
        xnose = joint_data_filtered[0][0]
        ## z nose
        znose = joint_data_filtered[0][2]
    if  type(joint_data_filtered[12])== list :
        ## x right shoulder
        xsh = joint_data_filtered[12][0]
        ## z right shoulder
        zsh = joint_data_filtered[12][2]
    ...
    # check if the joint was detected and has valid coordinate
        if joints_2D[joint_index].x <= 1 and joints_2D[joint_index].y <= 1 and joints_2D[joint_index].visibility >= 0.7 :
            ...
            if median_distance >= 0.3:
                #find joint coordinate with respect to the camera
...
def startend(q) :
    ...
    global act,qt1,qt2,act_index,dur,end,table_data
    ##act_index = [sit, stand, walk, sleeping, falling, no activity]
    ##activity_list = ['Sitting','Standing','Walking','Sleeping','Falling down','Waking up',' ']
    ##initial
    if act_index == [0,0,0,0,0,0]:
        i = activity_list.index(act)
        if i <= 5 :
            act_index[i] = 1
            qt1 = q[33]
            qt1str = timeformat(qt1[4],qt1[5],qt1[6]) 
            table_data.append([qt1str,act])
    else:
        ## if Activity isn't the same as the previous frame -->
        ##--> find the old Activity’s duration and timestamp the new Activity's time
        ## new activity
        iact = act_index.index(1) # to identify old activity
        ## if new activity != old activity 
        if activity_list.index(act) != iact:
            qt2 = q[33]
            ## time for changing activity >= 1 
            ## timedur = duration between the old and new act
            if timedur(qt1,qt2) >= 1 : 
                if iact == 5:
                    if table_data[-2][-1] == "Sleeping":
                        iact = 3
                    elif table_data[-2][-1] == "Falling down":
                        iact = 4
                dur[iact] += timedur(qt1,qt2)
                act_index[iact] = 0
                actn = activity_list.index(act)
                if actn <= 5:
                    act_index[actn] = 1
                    qt1 = q[33]
                    qt1str = timeformat(qt1[4],qt1[5],qt1[6]) 
...
def pieplot_color():
    global dur,colors, activity_count
...
def table():
    ...
    ax.axis('tight')
    ax.axis('off')
    ax.table(cellText=table_data,colLabels=column_labels,fontsize = 14,colColours=["plum"] * 3,cellLoc='center',loc="center")
    plt.show()
...
def check_num_joint(joint_data) :
    nd = 0
...
class RealSenseDepthStreamApp:
    ...
    def start_video_stream(self):
        ...
        global frame_count, act, bed_boundary, dur,colors,qt1,qt2,act_index,dur,end,table_data,in_bed,activity_count
        end = 0
        fig, ax = plt.subplots()
        activity_count = [0, 0, 0, 0, 0]
            ...
            min_tracking_confidence=0.5,model_complexity = 1) as pose:
                    ...
                    #cm.render_result(skeletons, color_image, joint_confidence)
                        ...
                        if not results.pose_landmarks:
                            frame_count += 1
                            no_skele_frame_count +=1
                            if no_skele_frame_count >= 4 and in_bed :
                                act = 'Sleeping'
                                #print(in_bed)
                                timesleep = [i for i in range(33)]
                                timesleep.append(timecheck())
                                startend(timesleep)
                                no_skele_frame_count = 0
    ...
    def start_rt_stream(self):
        global frame_count, act, bed_boundary, dur,colors,qt1,qt2,act_index,dur,end,table_data,in_bed,no_skele_frame_count
        end = 0
        fig, ax = plt.subplots()
        activity_count = [0, 0, 0, 0, 0]
        percent_activity = [0, 0, 0, 0, 0]
